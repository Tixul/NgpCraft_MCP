 ...
 *
 *  • First erase stub call of any session → always succeeds. ✓
 ...
 *   → ngpc_flash_erase_asm() uses the embedded AMD erase stub (no system.lib).
 *   → This is guaranteed to be the FIRST erase call of the session
 *     (no prior erase happened; 32 saves without power cycle
...
void ngpc_flash_save(const void *data)
{
    ...
    if (slot >= (u8)NUM_SLOTS) {
        /* Block full: erase via standalone AMD stub (first call of session). */
