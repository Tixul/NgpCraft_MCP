...
/* Called from VBlank ISR each frame to synchronise the per-line counter.
 * Automatically wired into ngpc_sys.c's isr_vblank. */
void ngpc_raster_vsync(void);

/* Diagnostic: number of ISR fires recorded last frame.
 * 152 = correct (one per scanline).  Read after ngpc_vsync(). */
extern volatile u8 ngpc_dbg_isr_fires;
...
/* Enable per-scanline road narrowing via palette c2 swap.
 * grass_c2: color for grass-colored bands (e.g. RGB(1,8,1))
 * road_c2:  color for road-colored bands  (e.g. RGB(5,5,5))
 * mid_thr:  scanline where mid  band turns road gray (~106)
 * out_thr:  scanline where outer band turns road gray (~128)
 * Palettes affected: SCR1 pal 4 c2 (HW_PAL_SCR1[18]) and pal 5 c2 ([22]).
 * Call once from scene_init() after ngpc_raster_init(). */
