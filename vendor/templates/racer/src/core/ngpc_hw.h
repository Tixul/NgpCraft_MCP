...
/* Interrupt vector level bytes.
 *
 * Each 4-byte interrupt vector entry = [addr_lo, addr_mid, addr_hi, level].
 * byte 3 = interrupt priority level (0 = disabled, 1-7 = enabled).
 * The BIOS sets VBlank level (0x6FCF = 4) but leaves Timer0 level
 * (0x6FD7) at 0 = disabled.  Must be written explicitly after installing
 * the Timer0 ISR address. */
#define HW_INT_VBL_LV   (*(volatile u8  *)0x6FCF)   /* VBlank level  (BIOS=4) */
#define HW_INT_TIM0_LV  (*(volatile u8  *)0x6FD7)   /* Timer0 level  (BIOS=0) */
